...
$(document).ready(function () {
  if($('a.sunMoon svg use').attr('xlink:href') == '#icon-sun') {
    $('.centerbg').css('background-image', 'url("' + bg[1] + '")')
  } else {
    $('.centerbg').css('background-image', 'url("' + bg[0] + '")')
  } 
})
    ...
    MN: function () {
      $('.iconflat').on('click', function () {
        if ($('#main-container').hasClass('open')) {
          $('.iconflat').css('width', '50px').css('height', '50px')
          $('.openNav').css('height', '50px').removeAttr('style')
        } else {
          $('.iconflat').css('width', '100%').css('height', '100%')
          $('.openNav').css('height', '100%').css('background', 'transparent')
        }
    ...
    AH: function () {
      ...
      $(window).scroll(function () {
        ...
        if (s == h1) {
          ...
          $('.openNav').removeClass('yya')
        }
        if (s > h1) {
          $('.site-header').addClass('yya')
          $('.openNav').addClass('yya')
        }
        if (s > h2) {
